-- Hydra Guard V4 · FRESH BOOTSTRAP
...
CREATE INDEX IF NOT EXISTS ix_damage_reports_owner_status
  ON damage_reports (owner_id, status);
CREATE INDEX IF NOT EXISTS ix_damage_reports_owner_created
  ON damage_reports (owner_id, created_at DESC);
CREATE INDEX IF NOT EXISTS ix_damage_reports_property
  ON damage_reports (property_id);
CREATE INDEX IF NOT EXISTS ix_damage_reports_tier
  ON damage_reports (claim_tier, status);
CREATE INDEX IF NOT EXISTS ix_assignments_sanierer_status
...
CREATE INDEX IF NOT EXISTS ix_activity_feed_report
  ON activity_feed (report_id, created_at DESC);
CREATE INDEX IF NOT EXISTS ix_photos_report
  ON damage_photos (report_id);
CREATE INDEX IF NOT EXISTS ix_properties_owner
  ON properties (owner_id);
CREATE INDEX IF NOT EXISTS ix_profiles_role
  ON profiles (role);
...
-- ===== 0003 =====
-- 0003_cause_and_pool.sql
-- Hydra Guard V4 – cause fields, sanierer pool stub, photo AI analysis.

-- ============================================================
-- Part A — damage_reports cause fields
-- ============================================================

ALTER TABLE damage_reports
  ADD COLUMN IF NOT EXISTS reported_cause text,
  ADD COLUMN IF NOT EXISTS confirmed_cause text,
  ADD COLUMN IF NOT EXISTS is_consequential_damage
    boolean NOT NULL DEFAULT false;


-- ============================================================
-- Part B — sanierer_pool_profiles stub
-- ============================================================

DO $$
BEGIN
  CREATE TYPE membership_tier AS ENUM ('basis', 'standard', 'premium');
EXCEPTION
  WHEN duplicate_object THEN NULL;
END;
$$;

CREATE TABLE IF NOT EXISTS sanierer_pool_profiles (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  profile_id uuid NOT NULL REFERENCES profiles(id)
    ON DELETE CASCADE,
  membership_tier membership_tier NOT NULL DEFAULT 'basis',
...
ALTER TABLE sanierer_pool_profiles ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Sanierer can read own pool profile"
  ON sanierer_pool_profiles FOR SELECT
  USING (profile_id = auth.uid());

CREATE POLICY "Sanierer can update own pool profile"
  ON sanierer_pool_profiles FOR UPDATE
  USING (profile_id = auth.uid());

CREATE POLICY "Sanierer can insert own pool profile"
  ON sanierer_pool_profiles FOR INSERT
  WITH CHECK (profile_id = auth.uid());


-- ============================================================
-- Part C — ai_analysis JSONB on damage_photos
-- ============================================================
