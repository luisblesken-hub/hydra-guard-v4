    ...
    <main className="mx-auto flex w-full max-w-5xl flex-col gap-6 px-6 py-8">
      <header className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        ...
        <nav className="flex gap-2 text-xs font-medium">
          <span className="rounded-full bg-slate-900 px-3 py-1 text-white">Fälle</span>
          <Link
          ...
          <ul className="space-y-3 md:hidden">
            {rows.map((row) => {
              const address = row.property
                ? [row.property.street, row.property.city].filter(Boolean).join(", ")
                : "—";
              return (
                <li
                  key={row.id}
                ...
                >
                  ...
                  <Link
                    href={`/claims/${row.id}`}
                    className="mt-3 inline-flex rounded-md border border-slate-200 px-2.5 py-1 text-xs font-medium text-slate-700 hover:bg-slate-50"
                  >
                    Details →
                  </Link>
                </li>
