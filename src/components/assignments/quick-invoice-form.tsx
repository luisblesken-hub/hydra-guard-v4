...
export function QuickInvoiceForm({
  ...
  if (state.success) {
    return <span className="text-xs text-hg-steel">✓ Rechnung eingereicht</span>;
  }
  ...
  if (!open) {
    return (
      <button
        ...
        className="inline-flex items-center rounded-md border border-indigo-300 bg-white px-2.5 py-1 text-xs font-semibold text-hg-steel hover:bg-indigo-50"
      >
        Rechnung einreichen
      </button>
    );
